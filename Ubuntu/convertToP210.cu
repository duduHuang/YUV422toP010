...
__global__ static void convertToP208Kernel(uint16_t *pV210, uint8_t *dP210, int nPitch, int nWidth, int nHeight, int *lookupTable) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    int tidd = blockIdx.y * blockDim.y + threadIdx.y;
    uint32_t v0, y0, u0, y2, u1, y1, u2, y3, v1, y5, v2, y4;
    uint4 pF;
    ...
    if (tid < nDstW && tidd < nDstH) {
        ...
        k = tid * 6;
        j = tidd * nPitch * 3 / 4;
        dP210[j + k + 0] = lookupTable[y0];
        dP210[j + k + 1] = lookupTable[y1];
        dP210[j + k + 2] = lookupTable[y2];
        dP210[j + k + 3] = lookupTable[y3];
        dP210[j + k + 4] = lookupTable[y4];
        dP210[j + k + 5] = lookupTable[y5];
        k = tid * 3;
        j = tidd * nPitch * 3 / 8 + nWidth * nHeight;
        dP210[j + k + 0] = lookupTable[u0];
        dP210[j + k + 1] = lookupTable[u1];
        dP210[j + k + 2] = lookupTable[u2];
        j = tidd * nPitch * 3 / 8 + nWidth * nHeight * 3 / 2;
        dP210[j + k + 0] = lookupTable[v0];
        dP210[j + k + 1] = lookupTable[v1];
        dP210[j + k + 2] = lookupTable[v2];
    }
}

void convertToP208(uint16_t *pV210, uint8_t *dP210, int nPitch, int nWidth, int nHeight, int *lookupTable, cudaStream_t stream) {
    dim3 blocks(32, 16, 1);
    dim3 grids((nPitch + blocks.x - 1) / blocks.x, (nHeight + blocks.y - 1) / blocks.y, 1);
    convertToP208Kernel << < grids, blocks, 0, stream >> > (pV210, dP210, nPitch, nWidth, nHeight, lookupTable);
}